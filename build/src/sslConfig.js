...
class SSLConfig {
    ...
    validateConfiguration() {
        ...
        if (this.validationMode === 'pinned' && this.certificateFingerprints.length === 0) {
            ...
            case 'pinned':
                return {
                    ...
                    checkServerIdentity: (hostname, cert) => {
                        ...
                        return 'Certificate fingerprint not in pinned list';
                    }
                };
    ...
    getConnectionStatus(host, success, error = null) {
        const icon = success ? '✅' : '❌';
        ...
        if (!success && error) {
            ...
            if (this.validationMode === 'strict' && error.message.includes('certificate')) {
                message += '\n   💡 Try SSL_VALIDATION=bypass if inverter uses self-signed certificates';
            }
            
            if (this.validationMode === 'pinned' && error.message.includes('fingerprint')) {
